...
import ProjectWebPro.jpa.exceptions.IllegalOrphanException;
import ProjectWebPro.jpa.exceptions.NonexistentEntityException;
import ProjectWebPro.jpa.exceptions.RollbackFailureException;
import ProjectWebPro.model.Account;
import java.io.Serializable;
import javax.persistence.Query;
import javax.persistence.EntityNotFoundException;
import javax.persistence.criteria.CriteriaQuery;
import javax.persistence.criteria.Root;
import ProjectWebPro.model.Register;
import java.util.ArrayList;
import java.util.List;
import ProjectWebPro.model.Customer;
...
public class AccountJpaController implements Serializable {
    ...
    public void create(Account account) throws RollbackFailureException, Exception {
        if (account.getRegisterList() == null) {
            account.setRegisterList(new ArrayList<Register>());
        }
        ...
        try {
            ...
            for (Register registerListRegisterToAttach : account.getRegisterList()) {
                registerListRegisterToAttach = em.getReference(registerListRegisterToAttach.getClass(), registerListRegisterToAttach.getRegisterId());
                attachedRegisterList.add(registerListRegisterToAttach);
            }
            account.setRegisterList(attachedRegisterList);
            ...
            em.persist(account);
            for (Register registerListRegister : account.getRegisterList()) {
                Account oldAccountIdOfRegisterListRegister = registerListRegister.getAccountId();
                registerListRegister.setAccountId(account);
                registerListRegister = em.merge(registerListRegister);
                if (oldAccountIdOfRegisterListRegister != null) {
                    oldAccountIdOfRegisterListRegister.getRegisterList().remove(registerListRegister);
                    oldAccountIdOfRegisterListRegister = em.merge(oldAccountIdOfRegisterListRegister);
                }
            }
    ...
    public void edit(Account account) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            List<Register> registerListOld = persistentAccount.getRegisterList();
            List<Register> registerListNew = account.getRegisterList();
            ...
            for (Register registerListOldRegister : registerListOld) {
                if (!registerListNew.contains(registerListOldRegister)) {
                    ...
                    illegalOrphanMessages.add("You must retain Register " + registerListOldRegister + " since its accountId field is not nullable.");
                }
            }
            ...
            for (Register registerListNewRegisterToAttach : registerListNew) {
                registerListNewRegisterToAttach = em.getReference(registerListNewRegisterToAttach.getClass(), registerListNewRegisterToAttach.getRegisterId());
                attachedRegisterListNew.add(registerListNewRegisterToAttach);
            }
            registerListNew = attachedRegisterListNew;
            account.setRegisterList(registerListNew);
            ...
            for (Register registerListNewRegister : registerListNew) {
                if (!registerListOld.contains(registerListNewRegister)) {
                    Account oldAccountIdOfRegisterListNewRegister = registerListNewRegister.getAccountId();
                    registerListNewRegister.setAccountId(account);
                    registerListNewRegister = em.merge(registerListNewRegister);
                    if (oldAccountIdOfRegisterListNewRegister != null && !oldAccountIdOfRegisterListNewRegister.equals(account)) {
                        oldAccountIdOfRegisterListNewRegister.getRegisterList().remove(registerListNewRegister);
                        oldAccountIdOfRegisterListNewRegister = em.merge(oldAccountIdOfRegisterListNewRegister);
                    }
                }
            }
            ...
            if (msg == null || msg.length() == 0) {
                ...
                if (findAccount(email) == null) {
                    throw new NonexistentEntityException("The account with id " + email + " no longer exists.");
                }
            }
            throw ex;
        } finally {
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            List<Register> registerListOrphanCheck = account.getRegisterList();
            for (Register registerListOrphanCheckRegister : registerListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Account (" + account + ") cannot be destroyed since the Register " + registerListOrphanCheckRegister + " in its registerList field has a non-nullable accountId field.");
            }
